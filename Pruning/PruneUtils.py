...
class WeightBias():
    def __init__(self, weight, bias, w_grad, b_grad):
        self.weight = weight
        self.bias = bias
...
class HeadAligned():
    def __init__(self, Q, K, V, proj):
        self.Q = Q
        self.K = K
        self.V = V
        self.proj = proj

def head_alignment(attn_block, n_heads, qk_dim, emb_in, v_dim, emb_out):
    qkv = attn_block.qkv
    ...
    qkv_grads_view = qkv.weight.grad.reshape(qkv_shape)
    q_weights_grad, k_weights_grad, v_weights_grad = qkv_grads_view[0], qkv_grads_view[1], qkv_grads_view[2]

    qkv_b_g_view = qkv.bias.grad.reshape(qkv_bias_shape)
    q_bias_grad, k_bias_grad, v_bias_grad = qkv_b_g_view[0], qkv_b_g_view[1], qkv_b_g_view[2]

    proj_grad = proj.weight.grad.reshape(proj_shape)
    proj_bias_grad = proj.bias.grad

    Q = WeightBias(q_weights, q_bias, q_weights_grad, q_bias_grad)
    K = WeightBias(k_weights, k_bias, k_weights_grad, k_bias_grad)
    V = WeightBias(v_weights, v_bias, v_weights_grad, v_bias_grad)
    proj = WeightBias(proj, proj.bias, proj_grad, proj_bias_grad)
